...
def _extract_monthly_rent(text: str) -> tuple[int | None, str, list[dict[str, Any]], list[str]]:
    ...
    if len(current_unique) > 1:
        evidence = [_evidence(value, "raw_explicit_current_monthly_rent", "high", excerpt, start, end) for value, excerpt, start, end in current_candidates]
...
def _extract_original_monthly_rent(text: str, current_rent: int | None) -> tuple[int | None, list[dict[str, Any]]]:
    ...
    unique = {value for value, _excerpt, _start, _end in candidates}
    evidence = [_evidence(value, "raw_explicit_original_monthly_rent", "high", excerpt, start, end) for value, excerpt, start, end in candidates]
    if len(unique) != 1:
...
def _extract_sale_price(text: str) -> tuple[int | None, str, list[dict[str, Any]], list[str]]:
    ...
    for match in re.finditer(pattern, text, flags=re.I):
        ...
        if parsed is not None:
            candidates.append((parsed, match.group(0), match.start(1), match.end(1)))
    unique = {value for value, _excerpt, _start, _end in candidates}
    evidence = [_evidence(value, "raw_explicit_sale_price", "high", excerpt, start, end) for value, excerpt, start, end in candidates]
    if len(unique) > 1:
...
def _extract_terms(text: str) -> tuple[dict[str, Any], dict[str, list[dict[str, Any]]]]:
    source = _normalize_cn_numbers(text)
    facts: dict[str, Any] = {"deposit_payment_terms": None, "deposit_months": None, "prepay_months": None, "contract_term_months": None, "contract_term_display": None}
...
def _extract_listing_details(text: str) -> tuple[dict[str, Any], dict[str, list[dict[str, Any]]]]:
    facts: dict[str, Any] = {"available_date": None, "management_fee": None, "internet_fee": None, "water_rate": None, "electric_rate": None, "parking_fee": None, "viewing_time": None, "video_viewing": None}
    evidence: dict[str, list[dict[str, Any]]] = {key: [] for key in facts}
    def store(key: str, match: re.Match[str] | None) -> None:
        ...
        if not value or any(token in value for token in ("待确认", "待定", "未知", "___", "【", "】")):
            return
        facts[key] = value[:40]
        evidence[key].append(_evidence(value[:40], f"raw_explicit_{key}", "high", match.group(0), match.start(1), match.end(1)))
    money_or_status = r"(已含|包含|免费|另付|不含|按表|\$?\s*\d+(?:\.\d+)?\s*(?:/|每)?\s*(?:m³|m3|立方米?|度|kwh|kw/h|月|辆)?)"
    patterns = {"management_fee": rf"(?:管理费|物业费)\s*[:：｜]?\s*{money_or_status}", "internet_fee": rf"(?:网络费|网费|网络|wifi)\s*[:：｜]?\s*{money_or_status}", "water_rate": rf"(?:水费)\s*[:：｜]?\s*{money_or_status}", "electric_rate": rf"(?:电费)\s*[:：｜]?\s*{money_or_status}", "parking_fee": rf"(?:停车费|停车|车位)\s*[:：｜]?\s*{money_or_status}"}
    for key, pattern in patterns.items():
        store(key, re.search(pattern, text, flags=re.I))
    store("available_date", re.search(r"(?:入住时间|可入住|入住)\s*[:：｜]?\s*([^\n；;]{1,24})", text, flags=re.I))
    store("viewing_time", re.search(r"(?:看房时间|可看房)\s*[:：｜]?\s*([^\n；;]{1,32})", text, flags=re.I))
    store("video_viewing", re.search(r"(?:视频看房|视频代看)\s*[:：｜]?\s*(可以安排|可安排|支持|可以|可)", text, flags=re.I))
    return facts, evidence


def _dimension(value: str) -> str | None:
    m = re.search(r"(\d+(?:\.\d+)?)\s*(?:m|米)\s*[x×*]\s*(\d+(?:\.\d+)?)\s*(?:m|米)", value, flags=re.I)
    return f"{m.group(1)}m×{m.group(2)}m" if m else None


def _extract_sizes(text: str) -> tuple[dict[str, Any], dict[str, list[dict[str, Any]]]]:
    facts: dict[str, Any] = {"size_sqm": None, "land_dimension": None, "building_dimension": None, "land_size_sqm": None, "building_size_sqm": None, "unlabelled_dimension": None}
    evidence: dict[str, list[dict[str, Any]]] = {key: [] for key in facts}
    unit = r"(?:㎡|平方米|m2|m²|sqm|sq\.?m)"
    for label, dimension_key, sqm_key in ((r"(?:土地|地块)(?:面积|尺寸)?", "land_dimension", "land_size_sqm"), (r"(?:建筑|建面)(?:面积|尺寸)?", "building_dimension", "building_size_sqm")):
    ...
    for line in str(text or "").splitlines():
        if re.search(r"(?:土地|地块|建筑|建面)", line, flags=re.I):
            continue
        sqm_match = re.search(r"(?:面积|size)\s*[:：]?\s*(\d+(?:\.\d+)?)\s*" + unit, line, flags=re.I) or re.search(r"(?<![\d.])(\d+(?:\.\d+)?)\s*" + unit, line, flags=re.I)
        if sqm_match:
            break
    if sqm_match:
...
def _extract_highlights(text: str) -> list[str]:
    rules = (("家具家电齐全", ("家具家电齐全", "家具齐全", "全新家具", "家具家电")), ("含保洁", ("包打扫", "含保洁", "cleaning")), ("含网络", ("包网络", "含网络", "wifi", "网络")), ("管家服务", ("管家服务",)), ("带泳池", ("泳池", "pool")), ("带健身房", ("健身房", "gym")), ("24H安保", ("24小时安保", "24h安保", "24h security", "security")), ("拎包入住", ("拎包入住",)), ("可停车", ("停车", "parking")))
    result: list[str] = []
...
def _special_tags(text: str) -> list[str]:
    tags: list[str] = []
    if re.search(r"特价|优惠", text): tags.append("special_price")
    if re.search(r"急租|急出|急转", text): tags.append("special_urgent")
    return tags
...
def _has_non_rental_intent(text: str) -> bool:
    ...
    for pattern in (r"(?:不考虑|暂无计划|不是|不|非|无意|谢绝|拒绝|不可|不能)\s*(?:对外)?\s*(?:急售|出售|售卖|转让|顶让|卖)", r"(?:not\s+for\s+sale|no\s+sale|rent(?:al)?\s+only|for\s+rent\s+only)"):
...
def _extract_deal_type(text: str) -> tuple[str, list[dict[str, Any]]]:
    rental, sale = _has_rental_intent(text), _has_non_rental_intent(text)
    if rental and sale: return "mixed", [_evidence("mixed", "raw_deal_terms", "high", "rent_and_sale_terms")]
    if rental: return "rent", [_evidence("rent", "raw_deal_terms", "high", "rental_terms")]
    if sale: return "sale", [_evidence("sale", "raw_deal_terms", "high", "sale_terms")]
    return "unknown", []
...
def _quality(facts: dict[str, Any]) -> dict[str, Any]:
    ...
    warning: list[str] = []
    info: list[str] = []
    if not facts.get("public_location_key"): hard.append("missing_public_location")
    if not facts.get("canonical_area_key"): info.append("geo_precision_unconfirmed")
    deal_type = str(facts.get("deal_type") or "unknown")
    if deal_type in {"rent", "mixed"} and not facts.get("monthly_rent_usd"): hard.append("missing_price")
    elif deal_type == "sale" and not facts.get("sale_price_usd"): hard.append("missing_sale_price")
    elif deal_type == "unknown" and not facts.get("monthly_rent_usd") and not facts.get("sale_price_usd"): hard.append("missing_price")
    ...
    if not facts.get("deposit_payment_terms"): warning.append("missing_deposit_terms")
    if not facts.get("contract_term_months"): warning.append("missing_contract_term")
    for tag in facts.get("special_tags") or []: info.append(tag)
    blocking = list(dict.fromkeys(hard + review))
    score = max(0, 100 - 30 * len(hard) - 12 * len(review) - 4 * len(warning))
    return {"hard_flags": list(dict.fromkeys(hard)), "review_flags": list(dict.fromkeys(review)), "warning_flags": list(dict.fromkeys(warning)), "info_flags": list(dict.fromkeys(info)), "blocking_flags": blocking, "all_flags": list(dict.fromkeys(hard + review + warning + info)), "score": score}
...
def canonicalize_source(raw_text: str, sanitized_text: str | None = None, source_identity: dict[str, Any] | None = None, media_summary: dict[str, Any] | None = None, manual_overrides: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    area_key, area_display = taxonomy.canonical_area_key, taxonomy.canonical_area_display
    project, project_alias, project_key, project_brand = taxonomy.project_name, taxonomy.project_alias, taxonomy.project_key, taxonomy.project_brand
    property_type, property_subtype, property_type_display, property_status = taxonomy.property_type, taxonomy.property_subtype, taxonomy.property_type_display, taxonomy.property_type_status
    ...
    for key, value in (manual_overrides or {}).items():
        ...
        if key in {"canonical_area_key", "canonical_area_display"}: facts["area_status"] = "confirmed" if facts.get("canonical_area_key") else "unconfirmed"
    public_key, public_display, publication_level = public_location_from_fields(canonical_area_key=facts.get("canonical_area_key"), canonical_area_display=facts.get("canonical_area_display"), area_status=facts.get("area_status"), market_location_keys=facts.get("market_location_keys"), market_location_displays=facts.get("market_location_displays"), project_key=facts.get("project_key"), project_name=facts.get("project_name"))
    facts["public_location_key"], facts["public_location_display"], facts["publication_location_level"] = public_key, public_display, publication_level
...
def draft_projection(facts: dict[str, Any]) -> dict[str, Any]:
    ...
    cost_notes = "；".join(f"{label}：{value}" for label, value in (("管理费", facts.get("management_fee")), ("网络", facts.get("internet_fee")), ("水费", facts.get("water_rate")), ("电费", facts.get("electric_rate")), ("停车", facts.get("parking_fee"))) if value)
    return {"title": facts.get("display_title") or "待确认房源", "deal_type": facts.get("deal_type") or "unknown", "project": facts.get("project_name") or "", "project_name": facts.get("project_name") or "", "project_alias": facts.get("project_alias") or "", "community": facts.get("community_name") or "", "city": facts.get("city_display") or CITY_DISPLAY, "area": facts.get("public_location_display") or "", "normalized_area": facts.get("canonical_area_key"), "public_location_key": facts.get("public_location_key"), "public_location_display": facts.get("public_location_display") or "", "publication_location_level": facts.get("publication_location_level") or "unknown", "property_type": facts.get("property_type") or "未知", "property_subtype": facts.get("property_subtype") or "", "property_type_display": facts.get("property_type_display") or facts.get("property_type") or "未知", "price": facts.get("monthly_rent_usd"), "original_price": facts.get("original_monthly_rent_usd"), "sale_price": facts.get("sale_price_usd"), "layout": facts.get("layout") or "", "size": (f"{facts['size_sqm']}㎡" if facts.get("size_sqm") is not None else ""), "land_size": facts.get("land_dimension") or (f"{facts['land_size_sqm']}㎡" if facts.get("land_size_sqm") is not None else ""), "building_size": facts.get("building_dimension") or (f"{facts['building_size_sqm']}㎡" if facts.get("building_size_sqm") is not None else ""), "unlabelled_dimension": facts.get("unlabelled_dimension") or "", "floor": facts.get("floor") or "", "deposit": payment, "payment_terms": payment, "contract_term": facts.get("contract_term_display") or "", "available_date": facts.get("available_date") or "", "management_fee": facts.get("management_fee") or "", "internet_fee": facts.get("internet_fee") or "", "water_rate": facts.get("water_rate") or "", "electric_rate": facts.get("electric_rate") or "", "parking_fee": facts.get("parking_fee") or "", "viewing_time": facts.get("viewing_time") or "", "video_viewing": facts.get("video_viewing") or "", "cost_notes": cost_notes, "highlights": list(facts.get("highlights") or []), "special_tags": list(facts.get("special_tags") or []), "quality_flags": list(quality.get("all_flags") or []), "blocking_quality_flags": list(quality.get("blocking_flags") or []), "quality_score": int(quality.get("score") or 0), "canonical_facts_hash": facts.get("canonical_facts_hash"), "normalized_data": facts}
