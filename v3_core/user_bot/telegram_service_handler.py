...
class TelegramServiceOutcome: handled:bool; action:str=''; rendered:bool=False; effect:ServiceEffectResult|None=None; ticket_id:int|None=None
...
def _draft_from_session(v):
    if not isinstance(v,dict) or not all(str(v.get(k) or '').strip() for k in ('issue_key','issue_label','request_token')):return None
    return ServiceRequestDraft(str(v['issue_key']),str(v['issue_label']),str(v.get('detail') or ''),str(v['request_token']))
def _store_draft(d,x): d[SERVICE_REQUEST_SESSION_KEY]={'issue_key':x.issue_key,'issue_label':x.issue_label,'detail':x.detail,'request_token':x.request_token}
...
def _requires_binding(a): return a in {'repair','property','tenant_lease','tenant_guide','tenant_handover','tenant_deposit','tenant_renew','tenant_terminate','tenant_renew_submit','tenant_terminate_submit','tenant_handover_pdf','tenant_deposit_pdf'} or a.startswith(('issue:','slot:'))
async def handle_v3_service_callback(update,context,*,service:TenantService,effects:ServiceEffectExecutor|None=None,advisor_url:str=''):
    ...
    a=raw[len('v3u:service:'):].strip(); data=getattr(context,'user_data',None)
    if not isinstance(data,dict):raise ValueError('telegram_user_data_missing_for_service')
    user=_lead_user(update); binding=service.active_binding(user.user_id); await q.answer()
    if _requires_binding(a) and binding is None: await render_service_view(q,tenant_home_view(service,user.user_id),advisor_url=advisor_url); return TelegramServiceOutcome(True,a,True)
    ...
    if a in views: await render_service_view(q,views[a](service,user.user_id),advisor_url=advisor_url); return TelegramServiceOutcome(True,a,True)
    if a in {'tenant_handover_pdf','tenant_deposit_pdf'}:
        await send_pdf(kind='handover' if a=='tenant_handover_pdf' else 'deposit',service=service,user_id=user.user_id,repo_root=Path(__file__).resolve().parents[2],context=context,chat_id=int(update.effective_chat.id)); return TelegramServiceOutcome(True,a,True)
    if a in {'tenant_renew_submit','tenant_terminate_submit'}:
        view,effect=await submit_request(kind='renew' if a=='tenant_renew_submit' else 'terminate',service=service,effects=effects,update=update,context=context); await render_service_view(q,view,advisor_url=advisor_url); return TelegramServiceOutcome(True,a,True,effect=effect)
    if a=='repair': await render_service_view(q,repair_home_view(),advisor_url=advisor_url); return TelegramServiceOutcome(True,a,True)
    if a=='property': await render_service_view(q,property_view(),advisor_url=advisor_url); return TelegramServiceOutcome(True,a,True)
    if a=='local': await render_service_view(q,local_life_view(),advisor_url=advisor_url); return TelegramServiceOutcome(True,a,True)
    if a=='general': await render_service_view(q,general_prompt_view(),advisor_url=advisor_url); data[SERVICE_GENERAL_WAIT_KEY]=True; data.pop(SERVICE_NEARBY_WAIT_KEY,None); return TelegramServiceOutcome(True,a,True)
    if a=='nearby': await render_service_view(q,nearby_view(),advisor_url=advisor_url); return TelegramServiceOutcome(True,a,True)
    if a=='nearby_other': await render_service_view(q,general_prompt_view(nearby=True),advisor_url=advisor_url); data[SERVICE_NEARBY_WAIT_KEY]=True; data.pop(SERVICE_GENERAL_WAIT_KEY,None); return TelegramServiceOutcome(True,a,True)
    if a=='rfcity': await render_service_view(q,rfcity_home_view(),advisor_url=advisor_url); return TelegramServiceOutcome(True,a,True)
    if a.startswith('rfcity:'): await render_service_view(q,_rfcity_category_product_view(a.split(':',1)[1]),advisor_url=advisor_url); return TelegramServiceOutcome(True,a,True)
    if a.startswith('issue:'):
        draft=service.begin_request(a.split(':',1)[1],request_token=uuid4().hex); await render_service_view(q,issue_prompt_view(draft),advisor_url=advisor_url); _store_draft(data,draft); data.pop(SERVICE_GENERAL_WAIT_KEY,None); data.pop(SERVICE_NEARBY_WAIT_KEY,None); return TelegramServiceOutcome(True,a,True)
    if a.startswith('slot:'):
        draft=_draft_from_session(data.get(SERVICE_REQUEST_SESSION_KEY))
        if draft is None or not draft.detail:return TelegramServiceOutcome(True,a,False)
        sub=service.submit_repair(user_id=user.user_id,draft=draft,slot=a.split(':',1)[1]); effect=await effects.repair(bot=getattr(context,'bot',None),user=user,submission=sub) if effects else None; await render_service_view(q,repair_success_view(urgent=sub.urgent),advisor_url=advisor_url); data.pop(SERVICE_REQUEST_SESSION_KEY,None); return TelegramServiceOutcome(True,a,True,effect,sub.ticket.id)
