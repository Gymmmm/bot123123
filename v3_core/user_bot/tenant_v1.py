...
def tenant_home_view(service:TenantService,user_id:int)->ServiceView:
    ...
    if b is None:
        return ServiceView('tenant_missing','🛠 <b>入住服务</b>\n\n当前账号还没有绑定有效租约。\n\n如果你已经通过侨联入住，请联系中文顾问核对并绑定入住档案。',((ServiceChoice('💬 中文顾问','v3u:home:contact'),),(ServiceChoice('🏠 返回首页','v3u:t:home'),)))
    return ServiceView('tenant_home',f'🛠 <b>入住服务</b>\n\n🏠 当前租约：<b>{he(b.property_name or "已绑定住房")}</b>\n\n这里可以查看租约、处理报修和物业事项，也可以查看租赁服务指南。\n\n续租或退租时，侨联会按当前租约继续衔接处理。',(
        (ServiceChoice('🔧 报修','v3u:service:repair'),ServiceChoice('🏢 物业沟通','v3u:service:property')),
        (ServiceChoice('📋 我的租约','v3u:service:tenant_lease'),ServiceChoice('📄 租赁服务指南','v3u:service:tenant_guide')),
...
def renew_view(service,user_id):
    ...
    return ServiceView('tenant_renew',f'🔄 <b>续租</b>\n\n🏠 当前房源：{he(b.property_name or "-")}\n📅 到期日：{he(end)}\n\n想继续住这套吗？\n\n确认后，中文顾问会和你核对新的租期和价格。',((ServiceChoice('✅ 我想续租','v3u:service:tenant_renew_submit'),),(ServiceChoice('💬 中文顾问','v3u:home:contact'),),(ServiceChoice('⬅️ 返回我的租约','v3u:service:tenant_lease'),)))

def terminate_view(service,user_id):
    b=service.require_active_binding(user_id); end=b.lease_end_date or b.contract_end_date or '待确认'
    return ServiceView('tenant_terminate',f'🚪 <b>退租</b>\n\n🏠 当前房源：{he(b.property_name or "-")}\n📅 合同到期：{he(end)}\n\n准备退租前，建议先确认合同中的通知期、押金退还条件和费用结算方式。\n\n提交退租后，中文顾问会与你确认预计退租时间、房屋交接和押金核对安排。',((ServiceChoice('✅ 我要退租','v3u:service:tenant_terminate_submit'),),(ServiceChoice('🔐 退租与押金说明','v3u:service:tenant_deposit'),),(ServiceChoice('💬 中文顾问','v3u:home:contact'),),(ServiceChoice('⬅️ 返回我的租约','v3u:service:tenant_lease'),)))
...
async def submit_request(*,kind,service,effects,update,context):
    user_obj=update.effective_user; user=LeadUser(int(user_obj.id),str(getattr(user_obj,'username','') or ''),str(getattr(user_obj,'full_name','') or '')); b=service.require_active_binding(user.user_id)
    ...
    if kind=='renew': text='💬 <b>续租正在跟进</b>\n\n中文顾问会继续和你确认新的租期和价格。' if duplicate else '✅ <b>已收到你的续租申请</b>\n\n中文顾问会和你确认新的租期和价格，再把结果告诉你。'
    else: text='💬 <b>退租正在跟进</b>\n\n中文顾问会继续与你确认退租交接和押金核对安排。' if duplicate else '✅ <b>已收到你的退租计划</b>\n\n中文顾问会与你确认预计退租日期、合同通知期、房屋交接、水电费用及押金核对事项。'
