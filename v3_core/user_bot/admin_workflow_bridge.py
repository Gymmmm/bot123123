"""V3-safe bridge for existing admin lead/repair workflow callbacks."""
...
from qiaolian_dual.admin_consult import format_lead_card, handle_admin_done
from qiaolian_dual.admin_contract import _is_admin_user
from qiaolian_dual.common import answer_callback_once, db, he
from qiaolian_dual.messages import advisor_response_notice_text, repair_progress_text
from qiaolian_dual.results_admin import admin_lead_keyboard
from qiaolian_dual.session_deeplink import user_display_name
...
async def handle_v3_admin_workflow(update, context, query, data: str, user):
            ...
            try:
                await context.bot.send_message(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [[InlineKeyboardButton("🛡 入住服务", callback_data="v3u:home:service")]]
                    ),
                )
            except Exception:
                pass
    ...
    card = re.sub(
        ...
        card,
        count=1,
    ) if "当前状态｜" in card else card.rstrip() + "\n\n" + status_line
    await query.edit_message_text(
        card,
        parse_mode=ParseMode.HTML,
