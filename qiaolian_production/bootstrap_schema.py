...
def ensure_runtime_schema(db_path: str | Path) -> None:
    path = Path(db_path)
    path.parent.mkdir(parents=True, exist_ok=True)

    # User-side tables are part of the same production database and must also
    # exist when Collector or Publisher is the first process started.
    try:
        from shared.db import Database as UserDatabase
    except ImportError:
        try:
            from .shared.db import Database as UserDatabase
        except ImportError:
