...
def _canonical_cover_fields(data: dict[str, Any]) -> dict[str, Any]:
    ...
    negotiable = price in {"售价面议", "租金面议", "价格面议", "面议"}
    if price and not price.startswith("$") and not negotiable:
        price = f"${price}"
    price_suffix = "/月" if deal_type == "rent" and not negotiable else ""
        ...
        try:
            ...
            poster_node.evaluate(
                r"""root => {
                    ...
                    if (mainTitle && areaValue && norm(mainTitle.textContent) && norm(mainTitle.textContent) === norm(areaValue.textContent)) {
                        const areaRow = areaValue.closest('[data-field]');
                        if (areaRow && /区域/.test(areaRow.textContent || '')) areaRow.style.display = 'none';
                    }
