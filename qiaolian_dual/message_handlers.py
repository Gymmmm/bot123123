...
async def handle_main_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if kctx is not None:
        if not text:
            await render_panel(update, text='请发一句关键词需求，例如：BKK1 预算800内 1房 安静。', reply_markup=main_keyboard(), context=context, prefer_edit_anchor=True)
            return MAIN
    ...
    if pref.get('goal') or pref.get('area'):
        await render_panel(update, text='当前流程是按钮选择，请直接点上面的区域/预算按钮继续。', reply_markup=main_keyboard(), context=context, prefer_edit_anchor=True)
        return MAIN
