...
async def _submit_appointment(
    ...
    *,
    ...
    新流程由“日期 → 时间”直接进入这里；历史 ``apconfirm:yes`` 仍复用
    同一提交函数，保证旧消息可继续使用，但新页面不再生成确认页。
    """
    ...
    subject_text = (
        ...
        else f"🏠 <b>{he(_title_layout_label(title, layout, '｜'))}</b>"
    )
    context.user_data.pop('appt', None)
    await respond(
        f"✅ <b>预约申请已提交</b>\n\n"
...
async def appoint_flow_cb(update: Update, context: ContextTypes.DEFAULT_TYPE, *, create_lead_fn=None, notify_admins_fn=None) -> int:
    ...
    if data.startswith('apmode:'):
        ...
        await query.edit_message_text(
            f'🎥 <b>{he(mode_label)}</b>\n\n请选择方便视频看房的日期。',
            reply_markup=_appointment_date_keyboard(show_video=False),
            parse_mode=ParseMode.HTML,
        )
        return APPT_DATE

    # 历史关注点按钮只做兼容落点；新 UI 已不再生成关注点页面。
    ...
    if data.startswith('apdate:'):
        ...
        if chosen == 'other':
            ...
            await query.edit_message_text(
                '📅 <b>请输入日期</b>\n\n例如：<code>0820</code>、<code>8月20日</code> 或 <code>下周三</code>',
                parse_mode=ParseMode.HTML,
            )
            return APPT_DATE
        appt['date'] = chosen
    ...
    # 旧确认页上的编辑按钮继续兼容，但新页面不会生成这些入口。
    if data == 'apedit:date':
        from .listing import listing_context
        info = listing_context(str(appt.get('listing_id') or ''))
        title = str(info.get('project') or info.get('title') or info.get('area') or '这套房')
        await query.edit_message_text(
            f'📅 <b>改预约日期 · {he(title)}</b>\n\n请重新选择：',
            parse_mode=ParseMode.HTML,
            reply_markup=_appointment_date_keyboard(),
        )
        return APPT_DATE
    ...
    if data == 'apedit:contact':
        ...
        await query.edit_message_text(
            _appointment_confirm_text(appt),
            reply_markup=_appointment_confirm_keyboard(),
            parse_mode=ParseMode.HTML,
        )
        return APPT_CONFIRM
    ...
    if data.startswith('aptime:'):
        ...
        return await _submit_appointment(
            update,
            context,
            appt,
            create_lead_fn=create_lead,
            notify_admins_fn=_notify_admins,
        )

    if data == 'appoint_back_time':
        query.data = 'apdate:' + appt.get('date', '')
        return await appoint_flow_cb(
            update,
            context,
            create_lead_fn=create_lead,
            notify_admins_fn=_notify_admins,
...
async def handle_appointment_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if appt.pop('awaiting_custom_date', False):
        ...
        if not normalized_date:
            ...
            await update.effective_message.reply_text(
                '日期格式没有识别出来。请试试：<code>0820</code>、<code>8月20日</code> 或 <code>下周三</code>。',
                parse_mode=ParseMode.HTML,
            )
            return APPT_DATE
        ...
        title = str(info.get('project') or info.get('title') or info.get('area') or '这套房')
        await update.effective_message.reply_text(
            f'📅 <b>{he(normalized_date)} · {he(title)}</b>\n\n请选择时间段：',
            parse_mode=ParseMode.HTML,
            reply_markup=_appointment_time_keyboard(),
        )
        return APPT_TIME
    ...
    if appt.pop('awaiting_custom_time', False):
        ...
        if not valid_time:
            appt['awaiting_custom_time'] = True
            await update.effective_message.reply_text(
                '时间格式没有识别出来。请试试：<code>20:00</code> 或 <code>晚上8点</code>。',
                parse_mode=ParseMode.HTML,
            )
            return APPT_TIME
        appt['time'] = value
    ...
    # 旧确认页可能留下 awaiting_contact；只做兼容，不再由新流程产生。
