...
async def appoint_flow_cb(update: Update, context: ContextTypes.DEFAULT_TYPE, *, create_lead_fn=None, notify_admins_fn=None) -> int:
    ...
    if data != 'home' and (not appt):
        await query.edit_message_text('这次预约已过期。\n\n请从房源帖里的「预约看房」重新进入，我会自动带上对应房源。', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('🏠 返回首页', callback_data='home')]]))
    ...
    if data.startswith('apdate:'):
        ...
        if chosen == 'other':
            ...
            await query.edit_message_text('📅 请直接输入日期\n\n例如：<code>0820</code> 或 <code>820</code>', parse_mode=ParseMode.HTML)
            return APPT_DATE
        appt['date'] = chosen
    ...
    if data.startswith('aptime:'):
        ...
        if chosen_time == 'other':
            ...
            await query.edit_message_text('⏰ 请直接输入方便的时间，例如：<code>20:00</code>', parse_mode=ParseMode.HTML)
            return APPT_TIME
    ...
    if data == 'apconfirm:yes':
        ...
        context.user_data.pop('appt', None)
        await query.edit_message_text(f"{_personal_greeting(update)}\n\n✅ <b>预约已提交</b>\n\n🏠 <b>{he(_title_layout_label(title, layout))}</b>\n🆔 <b>{_display_listing_id(lid_submit)}</b>\n📅 <b>{he(str(appt.get('date', '') or '-'))}</b>  ⏰ <b>{he(time_label)}</b>\n📞 联系：{he(contact)}\n\n侨联顾问会通过 Telegram 与您确认。\n⏱️ 预计 <b>30 分钟内</b>响应\n📞 急事可直接联系：<b>{he(str(ADVISOR_TG or '@pengqingw'))}</b>\n\n💎 <b>侨联地产</b>", parse_mode=ParseMode.HTML, reply_markup=appointment_success_keyboard())
        return MAIN
    if data == 'home':
...
async def handle_appointment_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if appt.pop('awaiting_custom_date', False):
        ...
        if not normalized_date:
            appt['awaiting_custom_date'] = True
            await update.effective_message.reply_text('日期格式没看懂，请直接输入例如：<code>0820</code> 或 <code>820</code>', parse_mode=ParseMode.HTML)
            return APPT_DATE
    ...
    if appt.pop('awaiting_custom_time', False):
        ...
        if not valid_time:
            appt['awaiting_custom_time'] = True
            await update.effective_message.reply_text('时间格式没看懂，请输入例如：<code>20:00</code> 或 <code>晚上8点</code>', parse_mode=ParseMode.HTML)
            return APPT_TIME
        appt['time'] = value
        info = listing_context(str(appt.get('listing_id') or ''))
        title = str(info.get('project') or info.get('title') or info.get('area') or '这套房')
