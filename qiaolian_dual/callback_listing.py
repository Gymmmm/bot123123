...
def matches(data: str) -> bool:
    return (data == 'find:show_more') or (data.startswith('listing:open:')) or (data.startswith('listing:appoint:')) or (data.startswith('listing:consult:')) or (data.startswith('listing:detail:')) or (data.startswith('listing:similar:'))
...
async def handle_listing_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, query, data: str, user) -> int | None:
    ...
    from .listing import _latest_listing_text, listing_context, listing_is_available, listing_unavailable_keyboard, listing_unavailable_text, start_video_tour_flow
    ...
    if data.startswith('listing:open:'):
            ...
            detail_text = listing_detail_text(item)
            detail_rows = [[InlineKeyboardButton('📅 预约看房', callback_data=f'listing:appoint:{listing_id}'), InlineKeyboardButton('💬 联系顾问', callback_data=f'listing:consult:{listing_id}')]]
