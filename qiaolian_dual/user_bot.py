...
from . import adapters as _adapters  # installs DeeplinkAdapter start-arg hook
...
from .message_handlers import handle_main_message, cmd_find, cmd_favorites, cmd_appointments, cmd_help, cmd_service, cmd_admin_list, cmd_admin_add, cmd_admin_remove, cmd_contact, cmd_search, cmd_about
...
async def start_appointment(update, context, listing_id, **kwargs):
    return await _start_appointment_impl(update, context, listing_id, render_panel_fn=render_panel, **kwargs)


async def appoint_flow_cb(update, context):
    return await _appoint_flow_cb_impl(update, context, create_lead_fn=create_lead, notify_admins_fn=_notify_admins)
...
async def start(update, context):
    return await _start_impl(update, context, upsert_user_profile_fn=upsert_user_profile, create_lead_fn=create_lead)
