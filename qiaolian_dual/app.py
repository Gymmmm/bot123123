...
def build_application(*, token: str | None = None) -> Application:
    ...
    # 归因接线只包现有 lead / 管理员通知；不改前台页面与公开 Deep Link。
    ...
    conv_handler = ConversationHandler(
        entry_points=[
            ...
            CommandHandler('deal_done', cmd_deal_done), CommandHandler('lead_response', cmd_lead_response), CommandHandler('repair_update', cmd_repair_update),
        ...
        states={
            ...
            APPT_FOCUS: [CallbackQueryHandler(appoint_flow_cb, pattern=_APPT_CB_PATTERN), CallbackQueryHandler(handle_ui_callback, pattern=_MAIN_CB_PATTERN)],
            APPT_DATE: [CallbackQueryHandler(appoint_flow_cb, pattern=_APPT_CB_PATTERN), CallbackQueryHandler(handle_ui_callback, pattern=_MAIN_CB_PATTERN), MessageHandler(filters.TEXT & ~filters.COMMAND, handle_appointment_text)],
            APPT_TIME: [CallbackQueryHandler(appoint_flow_cb, pattern=_APPT_CB_PATTERN), CallbackQueryHandler(handle_ui_callback, pattern=_MAIN_CB_PATTERN), MessageHandler(filters.TEXT & ~filters.COMMAND, handle_appointment_text)],
    ...
    if app.job_queue is not None:
        ...
        logger.warning('job_queue 不可用：租约到期前7天提醒任务未启动')
    return app
