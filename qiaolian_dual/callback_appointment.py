...
async def handle_appointment_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, query, data: str, user) -> int | None:
    ...
    if data.startswith('appointment_menu:contact'):
            ...
            await render_panel(update, text=advisor_handoff_text(listing_id=listing_id, user_id=user.id), parse_mode=ParseMode.HTML, reply_markup=contact_handoff_keyboard())
            return MAIN
