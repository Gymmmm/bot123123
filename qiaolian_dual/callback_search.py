...
async def handle_search_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, query, data: str, user, *, hooks: dict | None = None) -> int | None:
    ...
    if data == 'findmode:guided':
            ...
            await query.edit_message_text(smart_find_guided_header_text(), parse_mode=ParseMode.HTML, reply_markup=guided_search_keyboard())
            return MAIN
    if data.startswith('roompick:'):
            ...
            if matches:
                ...
                lines.append('\n如需更准，可以再补一个区域或预算。')
                await render_panel(update, text='\n'.join(lines), parse_mode=ParseMode.HTML, reply_markup=search_results_keyboard(matches))
            else:
                await render_panel(update, text=f'当前没有完全匹配 <b>{he(room_type)}</b> 的在架房源，我可以继续按区域或预算帮你缩小。', parse_mode=ParseMode.HTML, reply_markup=keyword_followup_keyboard(room_type=room_type))
            return MAIN
    if data == 'profile:repeat':
            binding_text, binding = old_tenant_binding_text(user.id)
            if not binding:
                context.user_data['awaiting_old_customer'] = True
                await render_panel(update, text='📋 <b>老客档案查询</b>\n\n请发送您的<b>姓氏 + 手机尾号 4 位</b>，或签约时使用的 Telegram 账号。\n\n例如：<code>彭 5678</code> 或 <code>@yourname</code>\n\n顾问核实后会帮您调取历史档案。\n🔐 信息仅用于内部核实。', parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('📞 联系顾问', callback_data='service:contact'), InlineKeyboardButton('⬅️ 返回', callback_data='service:hub')]]), context=context)
                return MAIN
    ...
    if data.startswith('findtype:'):
            ...
            await query.edit_message_text(f'🏠 <b>{he(goal_text)}</b>\n\n<b>第二步：请选择区域</b>\n全流程点击完成，不需要手动输入。', parse_mode=ParseMode.HTML, reply_markup=find_area_keyboard())
            return FIND_AREA
    if data.startswith('unavail:more:'):
            ...
            if matches:
                await render_panel(update, text=_format_listing_choice_lines(matches), parse_mode=ParseMode.HTML, reply_markup=search_results_keyboard(matches))
            else:
    ...
    if data == 'findback:area':
            ...
            goal_text = '我也说不清，直接帮我找' if goal == 'any' else goal
            await query.edit_message_text(f'🏠 <b>{he(goal_text)}</b>\n\n<b>第二步：请选择区域</b>', parse_mode=ParseMode.HTML, reply_markup=find_area_keyboard())
            return FIND_AREA
    if data.startswith('findarea:'):
            ...
            goal = str(pref.get('goal') or 'any')
            goal_text = '我也说不清，直接帮我找' if goal == 'any' else goal
            await query.edit_message_text(f'🏠 <b>{he(goal_text)}</b>\n📍 已选区域：<b>{he(area)}</b>\n\n<b>第三步：请选择预算区间</b>', parse_mode=ParseMode.HTML, reply_markup=find_budget_keyboard(goal))
            return FIND_BUDGET
