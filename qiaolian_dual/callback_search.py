...
def matches(data: str) -> bool:
    return (
        data in {'findmode:play', 'findmode:guided', 'profile:repeat', 'findback:area', 'find:similar'}
...
async def handle_search_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, query, data: str, user, *, hooks: dict | None = None) -> int | None:
    ...
    from .keyboards_common import keyword_followup_keyboard, no_match_followup_keyboard, old_tenant_followup_keyboard
    ...
    from .search import (
        create_lead as default_create_lead,
        detect_area,
        detect_property_type,
        search_listings_with_fallback as default_search_listings_with_fallback,
    ...
    if data == 'findmode:play':
        ...
        await query.edit_message_text(
            '🔍 <b>帮我找房</b>\n\n直接发一句需求就可以。\n例如：<code>BKK1 两房 800以内</code>',
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⬅️ 返回找房', callback_data='home_smart_search')]]),
        )
        return MAIN
    ...
    if data.startswith('roompick:'):
        ...
        matches_found, match_mode = search_listings_with_fallback(
        ...
        if matches_found:
            ...
            await render_panel(update, text=f'🔎 <b>暂时没有完全符合条件的房源</b>\n\n{he(room_type)}\n\n你可以调整一个条件继续找，\n也可以让中文顾问按这个需求继续留意。', parse_mode=ParseMode.HTML, reply_markup=no_match_followup_keyboard())
        return MAIN

    if data == 'profile:repeat':
        # 仅兼容历史按钮；新入住服务页面不再主动暴露自助绑定入口。
        binding_text, binding = old_tenant_binding_text(user.id)
        if not binding:
            await render_panel(
                update,
                text='📋 <b>我的租约</b>\n\n目前还没有绑定租约档案。\n\n如果是通过侨联入住的房源，联系中文顾问核对房号后即可补上。',
                parse_mode=ParseMode.HTML,
    ...
    if data.startswith('unavail:more:'):
        ...
        if matches_found:
            ...
            await render_panel(
                update,
                text=f"当前同区域（{he(area or '金边')}）暂时没有可预约房源。\n可以调整条件，或直接联系中文顾问。",
                parse_mode=ParseMode.HTML,
    ...
    if data.startswith('findarea:'):
        ...
        if code == 'other':
            await query.edit_message_text(
                '📍 <b>其他位置</b>\n\n直接输入区域、楼盘或附近地标。\n例如：<code>永旺2附近</code>',
                parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⬅️ 返回区域选择', callback_data='findback:area')]]),
            )
            return FIND_AREA
        ...
        await query.edit_message_text(
            f'💰 <b>每月预算大概多少？</b>\n\n单位：USD / 月\n已选：{he(area)}',
            parse_mode=ParseMode.HTML,
            reply_markup=find_budget_keyboard(str(pref.get('goal') or 'any')),
        )
        return FIND_BUDGET
    ...
    if data.startswith('findbudget:'):
        ...
        if code == 'other':
            ...
            await query.edit_message_text(
                ...
                parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⬅️ 返回预算选择', callback_data='findback:budget')]]),
            )
            return FIND_BUDGET
        ...
        search_snapshot = {'property_type': type_filter or None, 'area': area or None, 'budget_min': budget_min, 'budget_max': budget_max}
        context.user_data['last_search_pref'] = search_snapshot
        create_lead(
            user,
            ...
            payload={'goal': goal, 'area_hint': area, 'budget_label': budget_label, **(pref.get('touch_payload') or {})},
        )
        if _allow_admin_notify(context, key=f'search_activity:{int(user.id)}', cooldown_seconds=600):
            await _notify_admins(context, title='找房需求', lines=[f'用户：{_user_mention_html(user)}', f'联系方式：{he(_user_contact_text(user))}', f'类型：{he(goal)}', f"区域：{he(area or '-')}", f'预算：{he(budget_label)}'])
        matches_found, match_mode = search_listings_with_fallback(
    ...
    if data == 'find:similar':
        ...
        matches_found, mode = search_similar_listings(
        ...
        if matches_found:
            await send_find_results_as_cards(update, context, matches_found, mode)
        else:
