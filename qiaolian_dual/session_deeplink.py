...
def parse_start_arg_payload(arg: str) -> dict | None:
    ...
    if arg.startswith('photos__'):
        parts = arg.split('__', 2)
        token = parts[1] if len(parts) >= 2 else ''
        if token.startswith('ql'):
            return {'action': 'photos', 'target': '', 'post_token': token, 'opaque_token': token, 'channel_message_id': None}
        # 历史频道帖使用可逆的 base36 消息令牌，并把 listing_id
        # 放在第三段。保留该格式后，旧帖可直接进入同一套房的完整实拍，
        # 同时不改变新发布包的 ql... 不透明令牌协议。
    ...
    if arg.startswith('discussion_entry__'):
        ...
        if len(parts) == 3:
            ...
            # 新发布包使用 ql... 不透明令牌，它不是 base36 消息编号。
            # 只有历史可逆 post token 才可解码为 Telegram message_id，
            # 否则会生成超出 SQLite INTEGER 范围的假编号。
            channel_message_id = None if post_token.startswith('ql') else _base36_decode(post_token)
            return {'action': 'discussion_entry', 'target': listing_id, 'post_token': post_token, 'channel_message_id': channel_message_id}
...
def resolve_public_token(token: str) -> str:
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            # Durable compatibility registry for published channel links whose
            # package queue rows were removed during an operator rebuild.
            tables = {str(item[0]) for item in conn.execute("SELECT name FROM sqlite_master WHERE type='table'")}
            if 'public_link_registry' in tables:
                row = conn.execute(
                    """SELECT r.listing_id
                       FROM public_link_registry r
                       JOIN listings l ON l.listing_id=r.listing_id
                       WHERE r.public_token=? AND l.status IN ('active','reserved')
                       ORDER BY r.id DESC LIMIT 1""",
                    (token,),
                ).fetchone()
                if row:
                    return str(row[0] or '').strip()
            # Compatibility for already-published channel posts after an operator
            # intentionally rebuilds the pending/package tables. A prior deep-link
            # click records the exact opaque token -> listing mapping; only return it
            # while the listing is still bookable.
            row = conn.execute(
