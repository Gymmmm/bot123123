...
def _search_type_button_rows() -> list[list[InlineKeyboardButton]]:
    return [[InlineKeyboardButton('🏢 公寓 / 住宅', callback_data='findtype:住宅'), InlineKeyboardButton('🏡 别墅 / 排屋', callback_data='findtype:别墅/排屋')], [InlineKeyboardButton('🏬 写字楼 / 商铺', callback_data='findtype:商铺/办公'), InlineKeyboardButton('🤝 直接帮我找', callback_data='findtype:any')]]

def search_entry_keyboard() -> InlineKeyboardMarkup:
    rows: list[list[InlineKeyboardButton]] = [[InlineKeyboardButton('🎲 一句话关键词', callback_data='findmode:play'), InlineKeyboardButton('📍 按类型找（2～3步）', callback_data='findmode:guided')]]
...
def appointment_menu_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton('📅 预约实地看房', callback_data='appointment_menu:offline'), InlineKeyboardButton('🎥 视频代看', callback_data='appointment_menu:video')], [InlineKeyboardButton('📋 查看我的预约', callback_data='appointment_menu:list'), InlineKeyboardButton('💬 联系我们', callback_data='appointment_menu:contact')], [InlineKeyboardButton('🏠 返回首页', callback_data='home')]])
...
def service_hub_keyboard(user_id: int | None=None) -> InlineKeyboardMarkup:
    ...
    rows: list[list[InlineKeyboardButton]] = []
    first_label = '📋 我的租约' if binding else '🏠 我是侨联老客'
    first_data = 'contract:view' if binding else 'profile:repeat'
    rows.extend([[InlineKeyboardButton(first_label.replace('🏠 ', ''), callback_data=first_data), InlineKeyboardButton('🔧 报修', callback_data='service:repair_hub'), InlineKeyboardButton('🏢 物业', callback_data='service_request:property')], [InlineKeyboardButton('🔁 租约变更', callback_data='service:renew_change'), InlineKeyboardButton('🗺️ 周边生活', callback_data='service:local_life'), InlineKeyboardButton('💬 联系顾问', callback_data='service:contact')], [InlineKeyboardButton('🏠 返回首页', callback_data='home')]])
    return InlineKeyboardMarkup(rows)
...
def local_life_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton('🍴 餐厅', callback_data='rfcity:restaurant'), InlineKeyboardButton('🛒 超市', callback_data='rfcity:supermarket'), InlineKeyboardButton('🥤 饮品', callback_data='rfcity:drinks')], [InlineKeyboardButton('🚛 快递', callback_data='rfcity:logistics'), InlineKeyboardButton('💆 休闲', callback_data='rfcity:recreation'), InlineKeyboardButton('📋 全部分类', callback_data='local:rfcity')], [InlineKeyboardButton('⬅️ 返回入住服务', callback_data='service:hub')]])

def rfcity_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton('🍴 餐厅小吃', callback_data='rfcity:restaurant'), InlineKeyboardButton('🔥 烧烤夜宵', callback_data='rfcity:bbq')], [InlineKeyboardButton('🥤 奶茶饮品', callback_data='rfcity:drinks'), InlineKeyboardButton('🛒 超市便利', callback_data='rfcity:supermarket')], [InlineKeyboardButton('🏨 酒店租房', callback_data='rfcity:hotel'), InlineKeyboardButton('🏋️ 休闲生活', callback_data='rfcity:recreation')], [InlineKeyboardButton('🚛 快递物流', callback_data='rfcity:logistics'), InlineKeyboardButton('👨\u200d💻 富力物业', callback_data='rfcity:property')], [InlineKeyboardButton('🤝 商家入驻', callback_data='rfcity:join')], [InlineKeyboardButton('⬅️ 返回周边生活', callback_data='service:local_life')]])
