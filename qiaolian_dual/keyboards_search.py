...
def find_area_keyboard() -> InlineKeyboardMarkup:
    options = [
        ...
        ('koh', '钻石岛'), ('jinjie', '金街附近'), ('rf', '富力城'), ('aeon1', '永旺1附近'),
        ('tk', '堆谷（TK）'), ('russian', '俄罗斯市场附近'), ('pp', '炳发城'), ('chroy', '水净华半岛'), ('sen', '森速（永旺2一带）'),
    ]
    compact_pairs = options[:6]
    rows = [[InlineKeyboardButton(label, callback_data=f'findarea:{code}') for code, label in compact_pairs[index:index + 2]] for index in range(0, len(compact_pairs), 2)]
...
def _budget_options_for_goal(goal: str) -> list[tuple[str, str, int | None, int | None]]:
    return [('b1', '$400以内', None, 400), ('b2', '$400–600', 400, 600), ('b3', '$600–800', 600, 800), ('b4', '$800–1200', 800, 1200), ('b5', '$1200–1500', 1200, 1500), ('b6', '$1500+', 1500, None)]
...
def find_budget_keyboard(goal: str) -> InlineKeyboardMarkup:
    ...
    for index in range(0, len(options), 2):
        rows.append([InlineKeyboardButton(options[index][1], callback_data=f'findbudget:{options[index][0]}'), InlineKeyboardButton(options[index + 1][1], callback_data=f'findbudget:{options[index + 1][0]}')])
...
def appointment_menu_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton('📅 预约看房', callback_data='appointment_menu:offline')], [InlineKeyboardButton('📅 我的预约', callback_data='appointment_menu:list'), InlineKeyboardButton('💬 联系我们', callback_data='appointment_menu:contact')], [InlineKeyboardButton('⬅️ 返回首页', callback_data='home')]])
...
def precise_filter_keyboard(selected: set[str] | None=None) -> InlineKeyboardMarkup:
    ...
    def _btn(key: str) -> InlineKeyboardButton:
        ...
        return InlineKeyboardButton(('✅ ' if key in picked else '▫️ ') + label, callback_data=f'pref:toggle:{key}')
    return InlineKeyboardMarkup([[_btn('budget'), _btn('area')], [_btn('utility'), _btn('parking')], [_btn('quiet'), _btn('sunlight')], [_btn('pet'), _btn('furnished')], [_btn('chinese_owner'), _btn('amenity')], [InlineKeyboardButton('✅ 提交条件', callback_data='pref:submit'), InlineKeyboardButton('♻️ 清空', callback_data='pref:clear')], [InlineKeyboardButton('🏠 返回首页', callback_data='home')]])
...
def service_hub_keyboard(user_id: int | None=None) -> InlineKeyboardMarkup:
    ...
    if binding:
        rows.extend([
            ...
            [InlineKeyboardButton('🔄 续租', callback_data='contract:renew'), InlineKeyboardButton('🚪 退租 / 换房', callback_data='service:terminate')],
            [InlineKeyboardButton('🗺 周边生活', callback_data='service:local_life'), InlineKeyboardButton('💬 联系中文顾问', callback_data='service:contact')],
        ])
    else:
        rows.extend([
            [InlineKeyboardButton('📋 我的租约', callback_data='contract:view'), InlineKeyboardButton('📄 租赁服务指南', callback_data='hub:rental')],
            [InlineKeyboardButton('🔧 设备报修', callback_data='service:repair_hub'), InlineKeyboardButton('🏢 物业协调', callback_data='service_request:property')],
            [InlineKeyboardButton('🗺 周边生活', callback_data='service:local_life'), InlineKeyboardButton('💬 其他帮助', callback_data='service:general')],
...
def service_repair_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton('❄️ 空调', callback_data='service_request:repair_ac'), InlineKeyboardButton('🚿 热水器', callback_data='service_request:repair_water')], [InlineKeyboardButton('🧺 洗衣机', callback_data='service_request:repair_washer'), InlineKeyboardButton('🧊 冰箱', callback_data='service_request:repair_fridge')], [InlineKeyboardButton('📶 网络', callback_data='service_request:repair_network'), InlineKeyboardButton('🔐 门锁/门禁', callback_data='service_request:repair_door')], [InlineKeyboardButton('🔧 其他设备', callback_data='service_request:repair_other')], [InlineKeyboardButton('⬅️ 返回', callback_data='service:hub')]])


def service_detail_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton('💬 联系我们', callback_data='service:contact')], [InlineKeyboardButton('⬅️ 返回入住服务', callback_data='service:hub')]])


def local_life_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton('🧹 保洁家政', callback_data='service:contact'), InlineKeyboardButton('🚚 搬家协助', callback_data='hub:rental:moving')], [InlineKeyboardButton('🗺 周边推荐', callback_data='service:nearby'), InlineKeyboardButton('💬 其他需求', callback_data='service:general')], [InlineKeyboardButton('⬅️ 返回', callback_data='service:hub')]])


def nearby_area_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton('🏙 富力城导航', callback_data='local:rfcity')], [InlineKeyboardButton('📍 其他区域需求提交', callback_data='local:other')], [InlineKeyboardButton('⬅️ 返回生活服务', callback_data='service:local_life')]])


def rfcity_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton('🍴 餐厅小吃', callback_data='rfcity:restaurant'), InlineKeyboardButton('🔥 烧烤夜宵', callback_data='rfcity:bbq')], [InlineKeyboardButton('🥤 奶茶饮品', callback_data='rfcity:drinks'), InlineKeyboardButton('🛒 超市便利', callback_data='rfcity:supermarket')], [InlineKeyboardButton('🏨 酒店租房', callback_data='rfcity:hotel'), InlineKeyboardButton('🏋️ 休闲生活', callback_data='rfcity:recreation')], [InlineKeyboardButton('🚛 快递物流', callback_data='rfcity:logistics'), InlineKeyboardButton('👨‍💻 富力物业', callback_data='rfcity:property')], [InlineKeyboardButton('⬅️ 返回生活服务', callback_data='service:local_life')]])
