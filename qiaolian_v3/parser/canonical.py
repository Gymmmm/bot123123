...
_BUSINESS_FACT_KEYS = (
    "city_key", "city_display", "deal_type", "deal_type_candidates",
    "canonical_area_key", "canonical_area_display", "area_status", "canonical_area_level",
    "market_location_keys", "market_location_displays", "project_name", "project_alias",
    "project_key", "project_brand", "project_brand_key", "community_name", "property_type",
    "property_subtype", "property_type_display", "property_type_status", "layout", "bedrooms",
    "living_rooms", "bathrooms", "helper_rooms", "monthly_rent_usd",
    "original_monthly_rent_usd", "price_status", "sale_price_usd", "sale_price_status",
    "size_sqm", "land_dimension", "building_dimension", "land_size_sqm", "building_size_sqm",
    "unlabelled_dimension", "floor", "deposit_payment_terms", "deposit_months", "prepay_months",
    "contract_term_months", "contract_term_display", "available_date", "management_fee",
    "internet_fee", "water_rate", "electric_rate", "parking_fee", "viewing_time",
    "video_viewing", "highlights", "special_tags", "public_location_key",
    "public_location_display", "publication_location_level", "services", "included", "amenities",
    "rental", "house",
)


def canonical_business_projection(facts: dict[str, Any]) -> dict[str, Any]:
    """Stable business facts used for canonical identity; no operational evidence."""
...
def _normalize_deal_contract(facts: dict[str, Any]) -> None:
    ...
    if deal_type == "mixed":
        ...
        facts.setdefault("evidence", {})["deal_type"] = [{
            "value": ["rent", "sale"], "source": "raw_deal_terms", "confidence": "high",
    ...
    # Keep the existing Phase-3 convenience projections synchronized; these are
    # not part of the canonical business hash.
    facts["hard_flags"] = hard_flags
    facts["review_flags"] = review_flags
    facts["warning_flags"] = warning_flags
    facts["info_flags"] = info_flags
