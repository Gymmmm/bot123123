...
class ChannelEditor:
    ...
    def plan_edit(
        ...
        return ChannelEditPlan(
            action='edit', channel_id=str(channel_id), message_id=int(message_id),
            package_id=package.package_id, previous_content_hash=current_hash,
            next_content_hash=package.content_hash, dry_run=self.dry_run,
        )
