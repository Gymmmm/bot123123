...
def acceptance_candidates():
    return (
        candidate(101, message_id=5001, current_content_hash='before-hash-5001', caption='🏠 富力城｜1房\n💰 $800/月'),
        candidate(102, message_id=5002, current_content_hash='before-hash-5002', quality_result='ADMIN_APPROVED', caption='🏠 BKK1公寓｜2房\n💰 $950/月'),
        candidate(103, message_id=5003, current_content_hash='before-hash-5003', deal_type='sale', offer_type='sale', publication_policy='store_only', quality_result='ADMIN_APPROVED', package_id='PKG_SALE_103', caption='出售'),
        candidate(104, message_id=5004, current_content_hash='before-hash-5004', deal_type='unknown', publication_policy='store_only', quality_result='NEEDS_REVIEW', package_id='PKG_UNKNOWN_104', caption='待复核'),
        candidate(105, message_id=5005, current_content_hash='before-hash-5005', frozen=False, caption='未冻结'),
    )


def test_preview_generation_has_zero_telegram_writes_and_expected_blocks():
    preview = generate_preview(acceptance_candidates())
...
def test_each_manifest_record_is_human_inspectable_and_exactly_mapped():
    preview = generate_preview(acceptance_candidates())
    for record in preview.manifest:
        ...
        assert record.frozen_package_id.startswith('PKG_')
        assert record.public_ql_id.startswith('QL')


def test_target_set_is_frozen_and_any_data_or_mapping_drift_invalidates_preview():
    original = acceptance_candidates()
    ...
    for changed in drift_cases:
        current = (changed,) + original[1:]
        with pytest.raises(PreviewInvalid, match='PREVIEW_INVALID'):
            validate_preview(preview, current)
    with pytest.raises(PreviewInvalid, match='PREVIEW_INVALID'):
...
def test_read_only_db_loader_uses_frozen_package_and_does_not_change_publication_status():
    ...
    conn.execute("INSERT INTO v3_listings(id,public_listing_id,current_canonical_record_id,property_identity_key,listing_status) VALUES (1,'QL000201',1,'prop-1','active')")
    ...
    keyboard = [{'text': text, 'url': url} for text, url in buttons('QL000201')]
    ...
    assert preview.manifest[0].message_id == 8201
