...
def test_sale_catalog_filters_non_public_inventory(tmp_path: Path):
    ...
    _seed_listing(db, tmp_path, suffix="NEW", public_id="QL-3000", inventory_status="pending")
    ...
    assert set(items) == {"QL-3000", "QL-3001", "QL-3002"}
    assert items["QL-3000"]["status"] == "在售"
    assert items["QL-3001"]["status"] == "在售"
    assert items["QL-3002"]["status"] == "已预留"


def test_pending_sale_is_showcase_visible_without_rent_confirmation(tmp_path: Path):
    db = initialize_v3_storage(tmp_path / "v3.sqlite")
    _seed_listing(db, tmp_path, suffix="PEND", public_id="QL-PEND1", inventory_status="pending")
    repo = SaleCatalogRepository(db)
    payload = repo.list_sale_listings()
    assert payload["total"] == 1
    assert payload["items"][0]["public_id"] == "QL-PEND1"
    meta = repo.catalog_meta()
    assert meta["total"] == 1
    assert meta["available"] == 1
    assert repo.get_sale_listing("QL-PEND1")["public_id"] == "QL-PEND1"
