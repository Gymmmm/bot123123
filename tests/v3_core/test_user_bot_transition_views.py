...
def _book_plan(*, mode="offline"):
    return TransitionPlan(kind="book",next_step="appointment_date",effects=("render_appointment_date",),book=BookTransition(draft=PublicAppointmentDraft(public_listing_id=PUBLIC_ID,mode=mode,source="listing_callback")))
...
def test_book_view_matches_fixed_sha_date_entry_and_uses_public_identity_only():
    ...
    assert view.kind=="appointment_date"; assert "预约看房｜QL-RF-A2B3" in view.text; assert "富力城" in view.text; assert "$800/月" in view.text; assert "哪天方便看房" in view.text; assert "LST_1" not in view.text; assert inventory.calls==[PUBLIC_ID]
    assert [button.label for row in view.rows for button in row]==["今天","明天","后天","📅 其他日期","🎥 改为视频看房","⬅️ 返回租赁详情","🏠 返回首页"]
...
def test_video_mode_date_view_switches_fixed_sha_heading_question_and_toggle():
    view=TransitionViewService(InventoryStub(_published_view())).build(_book_plan(mode="video"),today=date(2026,9,8)); assert "视频看房｜QL-RF-A2B3" in view.text; assert "哪天方便视频看房" in view.text; toggle=view.rows[2][0]; assert toggle.label=="🚶 改为实地看房"; assert toggle.kind=="appointment_mode"; assert toggle.value=="offline"
...
def test_appointment_time_view_matches_fixed_sha_time_order():
    ...
    assert view.kind=="appointment_time"; assert "选择时间" in view.text; assert "9月10日" in view.text; assert "富力城" in view.text; assert "LST_1" not in view.text
...
def test_similar_view_preserves_fixed_sha_budget_order_and_frozen_area_only():
    ...
    assert view.kind=="search_budget"; assert "每月预算大概多少" in view.text; assert "已选：BKK1" in view.text; assert "LST_1" not in view.text
    assert [button.label for row in view.rows for button in row]==["$400以内","$400–600","$600–800","$800–1200","$1200–1500","$1500+","✍️ 自己输入","⬅️ 返回"]
    assert (view.rows[0][0].budget_min,view.rows[0][0].budget_max)==(None,400); assert (view.rows[2][1].budget_min,view.rows[2][1].budget_max)==(1500,None)
...
def test_change_search_view_matches_final_strict_entry_copy_and_button_order():
    ...
    assert view.kind=="search_entry"; assert "想找什么样的房子" in view.text; assert "BKK1 一房，预算 $600" in view.text
    labels=[button.label for row in view.rows for button in row]
    assert labels==["📍 按区域","💰 按预算","🏠 按户型","🏠 返回首页"]
