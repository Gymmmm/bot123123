...
from v3_core.publishing.autopilot_first_publish import AutoFirstPublishService
...
def pending_env(tmp_path, monkeypatch):
    ...
    service = AutoFirstPublishService(
        workflow=workflow,
        repository=repo,
...
def test_pending_auto_first_publish_success_activates_and_is_bookable(pending_env):
    ...
    pending_env.monkeypatch.setattr(
        "v3_core.publishing.autopilot_first_publish.deliver_approved_package", fake_deliver
    )
    pending_env.monkeypatch.setattr(
