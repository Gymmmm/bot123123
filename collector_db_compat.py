...
class DatabaseManager:
    ...
    def _ensure_optional_columns(self):
        ...
        # This compatibility manager is also imported by diagnostics and unit
        # tests that intentionally point at a new, empty database.  Core schema
        # creation belongs to schema_core.sql; never ALTER a table that does not
        # yet exist (SQLite reports "no such table: drafts").
        if not cols:
            return
        alters = []
        ...
        if "is_real_photo" not in cols:
            alters.append("ALTER TABLE drafts ADD COLUMN is_real_photo INTEGER NOT NULL DEFAULT 0")
        for sql in alters:
            cur.execute(sql)
        if alters:
            conn.commit()
