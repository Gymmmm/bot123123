...
function queryParams(){const p=new URLSearchParams({limit:LIMIT,offset}); const area=$('area').value,type=$('type').value,bed=$('bedrooms').value,price=$('price').value; if(area)p.set('area',area); if(type)p.set('property_type',type); if(bed)p.set('bedrooms',bed); if(price){const[min,max]=price.split('-');p.set('min_price',min);p.set('max_price',max)} return p}
async function loadMetadata(){if(metadataLoaded)return; const r=await fetch(API+'?limit=100&offset=0',{cache:'no-store'}); if(!r.ok)throw Error('metadata'); const d=await r.json(),areas=new Set(),types=new Set(),beds=new Set(); (d.items||[]).forEach(i=>{if(hasValue(locationOf(i)))areas.add(locationOf(i)); if(hasValue(i.property_type))types.add(text(i.property_type)); const b=bedroomsOf(i);if(b)beds.add(b)}); [...areas].sort().forEach(v=>$('area').insertAdjacentHTML('beforeend',`<option value="${escapeHtml(v)}">${escapeHtml(v)}</option>`)); [...types].sort().forEach(v=>$('type').insertAdjacentHTML('beforeend',`<option value="${escapeHtml(v)}">${escapeHtml(v)}</option>`)); [...beds].sort((a,b)=>a-b).forEach(v=>$('bedrooms').insertAdjacentHTML('beforeend',`<option value="${v}">${v}室</option>`)); metadataLoaded=true}
